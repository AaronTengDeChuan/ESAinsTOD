...
from src.utils.ontology import all_domains, db_domains
...
class MultiWozDB(object):
    def __init__(self, db_paths):
        ...
        for domain in all_domains:
            ...
            with open(db_paths[domain].replace("_processed", ""), 'r') as f:
                self.raw_dbs[domain] = json.loads(f.read())
    ...
    def get_match_num(self, constraints, return_entry=False, return_raw=False):
        """Create database pointer for all related domains."""
        match = {'general': ''}
        entry = {}
        # if turn_domains is None:
        ...
        for domain in all_domains:
            ...
            if domain in db_domains and constraints.get(domain):
                matched_ents = self.queryJsons(domain, constraints[domain], return_raw=return_raw)
    ...
    def queryJsons(self, domain, constraints, exactly_match=True, return_name=False, return_raw=False):
        """Returns the list of entities for a given domain
        based on the annotation of the belief state
        constraints: dict e.g. {'pricerange': 'cheap', 'area': 'west'}
        """
        # lower the constraints
        constraints = json.loads(json.dumps(constraints).lower())
        ...
        for v in constraints.values():
            if isinstance(v, str) and v not in ["not mentioned", ""]:
                valid_cons = True
        if not valid_cons:
            return []
        ...
        if 'name' in constraints and isinstance(constraints['name'], str):
            for idx, db_ent in enumerate(self.dbs[domain]):
                if 'name' in db_ent:
                    cons = constraints['name'].strip()
                    dbn = db_ent['name']
                    # if cons and set(cons.split()).issubset(set(dbn.split())):
                    if cons == dbn:
                        db_ent = db_ent if not return_name else db_ent['name']
                        match_result.append(db_ent)
                        raw_match_result.append(self.raw_dbs[domain][idx])
                        return match_result if return_name or not return_raw else raw_match_result
        ...
        for idx, db_ent in enumerate(self.dbs[domain]):
            ...
            for s, v in constraints.items():
                ...
                if s in ['arrive', 'leave']:
                    ...
                    # should satisfy inform_leave_time <= train_leave_time < train_arrive_time <= inform_arrive_time
        ...
        if not return_name:
            return match_result if not return_raw else raw_match_result
        else:
            if domain == 'train':
